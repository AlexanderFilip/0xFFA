...
void recompute_recent_cpu_time(){
	thread_current()->recent_cpu_time =
			(2*load_avg)/(2*load_avg + 1) * thread_current()->recent_cpu_time + thread_current()->nice
}


//recomputes threads priorities
void recompute_thread_priority(){
	thread_current()->priority  = PRI_MAX -
			(thread_current()->recent_cpu_time /4) - (2*thread_current()->nice);
}
...
{
  return (59/60)*load_avg + (1/60)*ready_threads;
}
...
thread_get_recent_cpu (void) 
{
	...
	return (2*load_avg)/(2*load_avg + 1) *
			thread_current()->recent_cpu_time + thread_current()->nice;
}

